...
static void select_col(uint8_t col) { writePinLow(col_pins[col]); }

static void unselect_col(uint8_t col) { writePinHigh(col_pins[col]); }
...
static void select_row(uint8_t row) { writePinLow(row_pins[row]); }
...
static void init_pins(void) {
    unselect_cols();
    for (uint8_t x = 0; x < MATRIX_ROWS; x++) {
        setPinOutput(row_pins[x], PAL_MODE_OUTPUT_PUSHPULL);
