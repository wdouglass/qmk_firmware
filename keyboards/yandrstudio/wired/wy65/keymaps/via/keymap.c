...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	LAYOUT(
		...
		KC_LCTL, KC_LGUI, KC_LALT,                   KC_SPC,           KC_VOLU, KC_VOLD, KC_RALT, KC_RCTL, KC_LEFT, KC_DOWN, KC_RIGHT),
	LAYOUT(
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    uint16_t keycode = 0;
    if (clockwise) {
        keycode = dynamic_keymap_get_keycode(biton32(layer_state), 4, 7);
    } else {
        keycode = dynamic_keymap_get_keycode(biton32(layer_state), 4, 8);
    }
